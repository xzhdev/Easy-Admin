<template>
  <div ref="menuRef" class="menu-wrapper" tabindex="0" @keyup="handleKeyUp">
    <ul class="menu-list" :style="getTheme">
      <li
        v-for="({ id, selected, name, subMenu, showSubMenu, disabled, divider, iconSlot }, index) of menuItems"
        :key="id"
      ...
      >
        <template v-if="!divider">
          <div :class="menuItemClass" @click="$event.stopPropagation()">
            ...
            <span v-if="subMenu" :class="['chev-icon', { disabled, 'show-submenu': showSubMenu }, menuStyle]">
...
import { PropType, ref, resolveComponent, computed, onMounted, watch, nextTick } from "vue";
...
const SubMenuComponent = resolveComponent("menuComponent");

const selectMenuItem = (name?: string, id?: string, subMenu?: boolean, selectFirstItem?: boolean) => {
  if (!subMenu) {
...
<style lang="scss" scoped src="./scss/Menu.scss"></style>
